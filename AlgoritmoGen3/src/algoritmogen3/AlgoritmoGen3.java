...
public class AlgoritmoGen3 {
    ...
    for (int x = 0; x < matriz.length; x++) {
                for (int y = 0; y < matriz[x].length; y++) {
                    double xr= Math.abs(li+((matriz2[x]/(Math.pow(2, size)-1))*(ls-li)));
                    matriz3[x]=xr;
                }
    }
    ...
    for (int x = 0; x < matriz.length; x++) {
                for (int y = 0; y < matriz[x].length; y++) {
                    t=matriz3[x];
                    fx=Math.abs(yo+(((m*g)/k)*t)-((((Math.pow(m,2))*g)/(Math.pow(k,2)))*(1-(Math.exp((-k*t)/m)))));
                    matriz4[x]=fx;
                }
    }
...
//x
    ...
    for (int x = 0; x < matrizH.length; x++) {
                    double xr= Math.abs(li+((matrizHBD[x]/(Math.pow(2, size)-1))*(ls-li)));
                    matrizHX[x]=xr;
